 ...
 * @auth HMAC-SHA256 (header: X-Webhook-Signature)
 */
...
try {
    ...
    // Validate HMAC (required for POST)
    ...
    error_log("[n8n/enrich] Received enrichment from workflow: $workflowId at $timestamp");
    ...
    foreach ($data['holdings'] as $enrichedHolding) {
        ...
        if (!$holding) {
            error_log("[n8n/enrich] Warning: ISIN $isin not found in portfolio");
            continue;
        }
